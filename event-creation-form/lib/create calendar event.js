...
function createCalendarEvent(eventInfo,instance,guestList) {
  ...
  Logger.log(allDates);
  var firstDate = new Date(allDates[0].toString().trim());
  ...
  Logger.log(endTime);
  if (guestList) {
    eventInfo.facEmails.forEach(function(f) {guestList.push(f)})
    var allGuests = guestList.join(",");
  } else {
    var allGuests = eventInfo.facEmails.join(",");
  }
  var eventDescription = "";
  Logger.log("event is public? "+eventInfo.public);
  eventInfo.public.indexOf('Yes') !== -1 ?
    eventDescription = eventInfo.description+"<br><br><a href='"+eventInfo.regUrl+"'>Use this link to register for the event(s)</a>"
  :
    eventDescription = "Registration for this event is by invitation only.";
  var newEvent = CalendarApp.getCalendarsByName(props[prefix+"_calendar_name"])[0]
                            .createEventSeries(eventInfo.title,startTime,endTime,CalendarApp.newRecurrence()
                                                                                            .addDate(firstDate),{description: eventDescription, location: eventInfo.location, sendInvites: true, guests: allGuests});
  if (allDates.length > 1) {
    //Logger.log("adding dates");
    for (var i=1;i<allDates.length;i++) {
      var thisDate = new Date(allDates[i].toString().trim());
