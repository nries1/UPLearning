function sendConfirmation(data, update, instance) {
  ...
  if (update) {
    subject = "Information has been edited for your event " + data.title;
  } //try {


  MailApp.sendEmail(data.facEmails, subject, body, {
    htmlBody: body
  }); //}
...
function emailBody(data, update, instance) {
  var prefix = boMap[instance].prefix; //var script = ScriptApp.getScriptId();
...
changes, you may do so <a href='" + data.edit_form + "'>here.</a></div>";
  ...
  if (data.fb_schedule === "every day") {
    var fbMsg = "This will be sent to participants automatically on the morning of each of your sessions";
  } else {
    var fbMsg = "This will be sent to participants automatically on the morning of the last date of your event.";
  }
  ...
  var body = "<div style='font-size: 1.4em'>\
