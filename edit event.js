function editEvent(id,form,instance) {
  ...
  row.push(form.ctle,form.funding,form.resource_folder,form["other_info"],form.display_audience,new Date(),form.framework.join(", "),form.guskey.join(", "),JSON.stringify(form.districts),form.support.join(", "),Session.getActiveUser(),form.waitlist,form.wl_size,form.feedback,"","","","");
  ...
  if (form.notify && Object.keys(form.notifyObj).length>0) {
    ...
    } catch (error) {allErrors.push({"title": "Registrant Notification", "error": error}); /*notifyError=error*/ } finally {}
  }
  try {
  ...
  try {
    ...
    var outObj = {"msg": returnMsg,"reg_link": regForm, "edit_link":editLink, "reg_flyer": flyerObj.url, "event_page": "", "event_id": id}; 
  } catch(error) {Logger.log(error);  return error;}
