...
function getAllEvents(instance) {
  ...
  var out = {
    events: [],
    districts: props["".concat(prefix, "_all_districts")].split(', ')
  };
  var database = new Spreadsheet(props[prefix + '_database_id'], ['event creation form responses']);
  var archive = new Spreadsheet(props[prefix + '_archive_id'], ['Events']);
  var dbs = [archive.data.Events, database.data['event creation form responses']];
  var event;

  for (var i = 0; i < dbs.length; i++) {
    for (var j = 1; j < dbs[i].length; j++) {
      if (dbs[i][j][eventCreatorIndex].toUpperCase() === userMail.toUpperCase() || dbs[i][j][firstFacEmailIndex].toUpperCase() === userMail.toUpperCase() || dbs[i][j][secondFacEmailIndex].toUpperCase() === userMail.toUpperCase() || dbs[i][j][thirdFacEmailIndex].toUpperCase() === userMail.toUpperCase() || userMail.toLowerCase() === props["".concat(prefix, "_dm_email")].toLowerCase() || userMail.toLowerCase() === 'professionallearning@strongschools.nyc' || dbs[i][j][directorEmailIndex].toUpperCase() === userMail.toUpperCase()) {
...
function Event(data, archive, instance) {
  ...
  if (data[84] === 1) {
    ...
    this.dates = data[dateIndex].toString().slice(0, 15);
    +" <span style='color: #f05537;'>through</span> " + data[firstDateIndex + Number(data[84]) - 1].toString().slice(0, 15);
