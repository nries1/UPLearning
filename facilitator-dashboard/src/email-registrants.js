function emailRegistrants(eventObj,instance) {
  ...
  if (regEmails.length === 1) {return "Nobody registered for this event. So no emails were sent."}
  try {
  ...
  MailApp.sendEmail(userMail, "Professional Learning Session Canceled", emailBody, {htmlBody: emailBody, cc: regEmails.join(",")});
