...
function getArchvieRegDetails(eventId,instance) {
  ...
  var regDetailsObj = {"participants": [], "waitlist": 0, "sent_ctle": 0}
  for (var i=0; i<archiveRegData.length; i++) {
    if (archiveRegData[i][r_eventIdIndex] === eventId) {
      ...
      if (archiveRegData[i][r_sentCtleIndex].indexOf("Sent")!==-1) {regDetailsObj.sent_ctle++}
...
function getNumRegs(eventId,regData) {
  ...
  for (var i=0; i<regData.length; i++) {
    if (regData[i][r_eventIdIndex] === eventId) {
      if (regData[i][r_waitlistIndex] == "Waitlist") {waitlist++;}
      else {numRegs++;}
    }
  }
  return {"numRegs": numRegs, "waitlist": waitlist}
}
